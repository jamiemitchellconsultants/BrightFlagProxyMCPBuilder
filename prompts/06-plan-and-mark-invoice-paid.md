...
- issues a plan token generated by a cryptographically secure random source with at least 128 bits
  of entropy, opaque to the caller and encoding none of the bound fields, with the binding to
  caller, tenant, invoice identity, amount, currency, `paymentRef`, and contract version held only
  in the server-side plan store record it references, expiring after a short configured lifetime.
...
- plan tokens are drawn from a cryptographically secure random source, with no observable
  correlation between issued tokens and no plausible path to guess or enumerate a valid one;
...
- The plan token is an opaque random capability, not a signed or self-describing structure; the
  bound fields live only in the server-side plan store.
